import { NextRequest } from "next/server";
import { ok, fail } from "@/lib/api";
import { requireBearer } from "@/lib/auth";
import { DATABASE_ENV_KEYS, formatMissingEnvError } from "@/lib/env";
import { markProspectReplied } from "@/lib/prospects";

export const runtime = "nodejs";

type ProspectRouteContext = {
  params: Promise<{
    id: string;
  }>;
};

export async function POST(request: NextRequest, context: ProspectRouteContext) {
  const authError = requireBearer(request);

  if (authError) {
    return authError;
  }

  const configError = formatMissingEnvError("la base de datos", DATABASE_ENV_KEYS);

  if (configError) {
    return fail("DATABASE_CONFIG_MISSING", configError, 503);
  }

  const payload = (await request.json().catch(() => ({}))) as {
    source?: unknown;
    messageId?: unknown;
    snippet?: unknown;
    receivedAt?: unknown;
  };

  try {
    const { id } = await context.params;
    const item = await markProspectReplied(id, {
      source: typeof payload.source === "string" ? payload.source : undefined,
      messageId: typeof payload.messageId === "string" ? payload.messageId : undefined,
      snippet: typeof payload.snippet === "string" ? payload.snippet : undefined,
      receivedAt: typeof payload.receivedAt === "string" ? payload.receivedAt : undefined,
    });

    return ok({
      item,
      result: {
        id: item.id,
        status: item.status,
        hotLead: item.hotLead,
      },
    });
  } catch (error) {
    return fail(
      "PROSPECT_REPLY_FAILED",
      error instanceof Error ? error.message : "No se pudo registrar el reply.",
      400
    );
  }
}
